#include "interaction/poisson_fc.h"
#include "fftw.h"
#include "fftw3.h"
...
void FastConvolution::solve(blaze::DynamicVector<double> &V,
    ...
    auto G_l = -1.0 / (mu_l * mu_l);

    // Uniform sums, i.e. independent of x, can be computed once
    double S1 =
        sum(source_k * (box.xmin + cos(M_PI * l) * box.xmax) / (2 * mu_l));
    double S2 = sum(source_k * (1.0 + cos(M_PI * l)) / (2 * mu_l));

    // x dependent sum is accelerated with DST
    source_k *= 0.5 * G_l;
    fftw_execute(dst.get());

    V = -S1 + x_j * S2 + source_k;
}
