#include "evolution/uso_kdk_itp.h"
#include "cosmology.h"
#include "io.h"
...
USO_KDK_ITP::USO_KDK_ITP(const Parameters& p, const SimState& state,
    ...
    : DefaultDriver{p},
      ...
      norm0{sqrt(L / N * sum(real(conj(state.psi) * state.psi)))},
      k_squared(N),
      kick(N),
      dt_last{-1} {
...
void USO_KDK_ITP::step(SimState& state, const double dt) {
    ...
    auto drift = exp(-1.0 * a_half * state.V * dt);
    psi *= drift;
    ...
    double norm2 = L / N * sum(real(conj(state.psi) * state.psi));

    // Renormalize to box size
    psi /= sqrt(norm2) / norm0;
