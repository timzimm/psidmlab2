...
PhaseSpaceDistribution::compute(const SimState& state) {
    ...
    // increase preformance (potentially significantly). The downside is that we
    // either have to store two matrices to do the out of place FFT, or have one
    // padded matrix for which we have to mess with C++ type system to
    // accomodate std::complex and doubles at once. I don't know if this is
    // really the right approach and worth the effort.
    ...
    for (int n = 0; 2 * n < N; ++n) {
        ...
        // Even columns
        // conj switched due to sign convention of FFTW. Result is real so no
        // additional conj required.
        lag = 1.0 / (2 * N) * conj(sig_npk) * sig_nmk;

        fftw_execute_dft_c2r(even_plan, lag_ptr,
                             W.data() + 2 * n * W.spacing());

        // Odd columns
        lag = 1.0 / (2 * N) * sig_npk * conj(sig_nmk1);
